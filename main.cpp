...
public:
    ...
    void Update(){
        if(type==PlatformType::MOVING){
            rect.x+=moveDir*2.0f;
            if(rect.x<=0 || rect.x+rect.width>=Config::SCREEN_W)
                moveDir*=-1;
        }
    ...
    void Draw() const{
        ...
        Color col=
        (type==PlatformType::NORMAL)?DARKGREEN:
...
public:
    ...
    void InitLevel(){
        ...
        for(int i=1;i<Config::PLATFORM_COUNT;i++){
            float rx=(float)GetRandomValue(
                0,
                Config::SCREEN_W-Config::PLATFORM_W
            );

            float ry=(Config::SCREEN_H-100.0f)
                     -(i*Config::PLAT_SPACING);

            platforms.push_back(
                Platform(rx,ry,PlatformType::NORMAL)
            );
        }
    }
    ...
    void UpdatePlaying(){
        ...
        float screenBottom=
        camera.target.y+(Config::SCREEN_H/2.0f);
        ...
        if(playerPos.y>screenBottom+200)
    ...
    void Draw(){
        ...
        if(state==GameState::LOGIN){
            ...
            DrawText(
                TextFormat("PLAYER: %s",usuario),
                10,15,18,BLACK
            );

            DrawText(
                TextFormat("SCORE: %05i",score),
                200,15,18,MAROON
            );

            DrawText(
                TextFormat("TOKENS: %d",tokensJugador),
                10,35,14,GOLD
            );
